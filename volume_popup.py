...
from PyQt6.QtWidgets import (
    ...
    QSlider, QPushButton, QApplication, QTabWidget, QCheckBox, 
    QComboBox, QSpinBox, QGroupBox, QButtonGroup, QRadioButton, QGridLayout
)
...
class VolumePopup(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.setMinimumWidth(380) # Reduced width for better fit
        self.adjustSize()
    ...
    def _update_label(self, value):
        ...
        level = "normal"
        if value > 150: level = "danger"
        elif value > 100: level = "warning"
...
class VolumeButton(QPushButton):
    ...
    def show_popup(self):
        ...
        target_x = max(screen_geo.left() + 10, min(target_x, screen_geo.right() - pw - 30))
        target_y = max(screen_geo.top() + 10, min(target_y, screen_geo.bottom() - ph - 10))
    ...
    def _update_icon(self, value):
        if value == 0: self.setIcon(self.icons['volume_mute'])
        elif value < 33: self.setIcon(self.icons['volume_low'])
        elif value < 66: self.setIcon(self.icons['volume_medium'])
        else: self.setIcon(self.icons['volume_hight'])
