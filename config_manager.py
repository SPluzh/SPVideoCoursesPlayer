...
class ConfigManager:
    ...
    def get_excluded_library_paths(self) -> set:
        ...
        return {os.path.normpath(p.strip()) for p in excluded_str.split(';') if p.strip()}
    ...
    def get_thumbnails_dir(self) -> Path:
        ...
        if config.has_section('Paths'):
            thumbnails_dir = Path(config.get('Paths', 'thumbnails_dir', fallback=default))
    ...
    def save_filter_state(self, fav_active: bool, tag_active: bool, selected_tag_ids: set):
